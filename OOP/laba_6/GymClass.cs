...
namespace laba_6
{
    ...
    internal class GymController
    {
        ...
        {
            if (budget < 0) throw new CustomException("Ошибка! Бюджет не может быть отрицательным.");
            this.budget = budget;
        ...
        public void AddInventory(Inventory item)
        {
            try
            {
                foreach (var invent in gym.Inventory)
                {
                    if (item.Name == invent.Name)
                    {
                        throw new InventoryDuplication($"Ошибка! Инвентарь с названием '{item.Name}' уже добавлен.");
                    }
                }
